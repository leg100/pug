...
type RowRenderer[T any] func(T, lipgloss.Style) RenderedRow
...
func New[T Item](columns []Column, fn RowRenderer[T], width, height int) Model[T] {
	m := Model[T]{
		cursor:      0,
...
func (m Model[T]) Highlighted() (T, bool) {
	...
	return m.getItem(m.cursor), true
}
...
func (m *Model[T]) SetItems(items []T) {
	...
	m.rows = make([]resource.ID, 0, len(items))
...
func (m Model[T]) getItem(rowID int) T {
	resourceID := m.rows[rowID]
	return m.items[resourceID]
}

func (m *Model[T]) renderRow(rowID int) string {
	var background lipgloss.Style
	if _, ok := m.Selected[m.rows[rowID]]; ok {
	...
	item := m.getItem(rowID)
	cells := m.rowRenderer(item, background)
