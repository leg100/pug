...
func (t *Tree) InsertChild(child any, pos int) *Tree {
	switch item := child.(type) {
	case *Tree:
		t.children = t.children.(NodeChildren).Append(nil)
		copy(t.children.(NodeChildren)[pos+1:], t.children.(NodeChildren)[pos:])
		t.children.(NodeChildren)[pos] = item
	case Children:
		for i := 0; i < item.Length(); i++ {
			t.children = t.children.(NodeChildren).Append(item.At(i))
		}
